...
void quickSort(int*, int, int); // 재귀 호출 방식을 통해 퀵 정렬을 수행하는 함수
void swap(int*, int*); // 배열의 원소 위치를 바꾸기 위한 함수
int partition(int*, int, int); // 배열의 pivot을 중심으로 해당 단계의 퀵정렬을 수행하는 함수. (Divide)
...
int partition(int *arr, int left, int right){
	int pivot=*(arr+right), l=left-1, r=right;
